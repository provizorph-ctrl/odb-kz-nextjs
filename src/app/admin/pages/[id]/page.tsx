...
export default function EditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [form, setForm] = useState({
    ...
    description: "", isPublished: false, sortOrder: 0,
  });
  ...
  useEffect(() => {
    fetch(`/api/pages/${id}`).then(r => r.json()).then(data => {
      setForm({
        ...
        contentKz: data.contentKz || "", contentQz: data.contentQz || "",
        description: data.description || "", isPublished: data.isPublished || false,
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">Редактирование страницы</h1>
