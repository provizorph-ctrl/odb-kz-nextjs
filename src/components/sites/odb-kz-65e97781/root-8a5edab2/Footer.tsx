        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-8 mb-6 sm:mb-8">
          <div className="col-span-2 md:col-span-1 space-y-3 sm:space-y-4">
            <div className="flex items-center gap-2.5 sm:gap-3">
              <div className="size-9 sm:size-10 rounded-xl overflow-hidden shrink-0">
              ...
              <div className="min-w-0">
                ...
                <div className="text-[10px] sm:text-xs text-white/60 leading-tight truncate">
            ...
            <div className="flex gap-1.5 sm:gap-2">
              {socialLinks.map((social) => (
                <a
                  key={social.platform}
                  href={social.url}
                  className="size-8 sm:size-9 flex items-center justify-center rounded-lg bg-white/10 hover:bg-white/20 transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white touch-manipulation"
          ...
          <div>
            <h4 className="text-xs sm:text-sm font-semibold mb-2.5 sm:mb-4 flex items-center gap-2 font-[family-name:var(--font-heading)]">
              <div className="w-4 sm:w-5 h-0.5 bg-primary rounded-full" aria-hidden="true" />
              {t("contactsTitle")}
            </h4>
            <div className="space-y-2 sm:space-y-3">
              {contacts.map((contact) => (
                <div key={contact.label}>
                  <span className="text-[10px] sm:text-xs text-white/50 block">{contact.label}</span>
                  <span className="text-xs sm:text-sm font-medium">{contact.value}</span>
                </div>
              ))}
            </div>
          </div>
