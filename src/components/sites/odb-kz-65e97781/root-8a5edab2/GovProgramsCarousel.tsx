    ...
    <section className="py-4">
      <div className="flex items-center justify-between mb-4 sm:mb-5">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base sm:text-xl font-bold">Государственные программы</h2>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={prev} disabled={currentIndex === 0} className="size-8 sm:size-9 flex items-center justify-center rounded-full border border-border hover:border-primary hover:text-primary transition-all disabled:opacity-30" aria-label="Назад">
          ...
          <button onClick={next} disabled={currentIndex >= maxIndex} className="size-8 sm:size-9 flex items-center justify-center rounded-full border border-border hover:border-primary hover:text-primary transition-all disabled:opacity-30" aria-label="Вперёд">
        ...
        <div className="flex transition-transform duration-500 ease-out" style={{ transform: `translateX(-${currentIndex * (100 / visibleCount)}%)` }}>
          {programs.map((program) => (
            <div key={program.id} className="flex-shrink-0 px-1.5 sm:px-2" style={{ width: `${100 / visibleCount}%` }}>
              <a href={program.url} className="group block rounded-xl sm:rounded-2xl overflow-hidden bg-white border border-border/50 shadow-card hover:shadow-card-hover transition-all duration-300 hover:-translate-y-1">
                ...
                <div className="p-3 sm:p-4">
                  <span className="text-xs sm:text-sm font-semibold text-foreground group-hover:text-primary transition-colors line-clamp-2">{program.title}</span>
                </div>
              </a>
            </div>
          ))}
      ...
      <div className="flex justify-center gap-2 mt-4">
        {programs.map((_, index) => (
          <button key={index} onClick={() => goTo(index)} className={`h-2 rounded-full transition-all duration-300 ${index === currentIndex ? "bg-primary w-6 sm:w-8" : "bg-primary/20 w-2 hover:bg-primary/40"}`} aria-label={`Слайд ${index + 1}`} />
        ))}
      </div>
    </section>
  );
