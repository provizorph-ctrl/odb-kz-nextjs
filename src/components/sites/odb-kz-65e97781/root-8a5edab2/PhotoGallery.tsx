...
const categories: GalleryCategory[] = [
  { id: "all", name: "О поликлинике", isActive: true },
  { id: "staff", name: "Персонал" },
  { id: "recommendations", name: "Рекомендации специалистов" },
    ...
    <section className="py-4">
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-xl font-bold">Фотогалерея</h2>
        </div>
        <a href="#" className="text-sm text-primary hover:text-primary/80 font-medium transition-colors flex items-center gap-1">
          Все фото
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
        {filteredImages.map((image, index) => (
          <a
            key={image.id}
            href={image.src}
            className="group relative aspect-square rounded-xl overflow-hidden bg-muted border border-border/50 hover:border-primary/30 transition-all duration-300 hover:shadow-card-hover"
