    ...
    <section className="py-3 sm:py-4">
      <div className="flex items-center justify-between mb-3 sm:mb-5">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base sm:text-2xl font-bold text-foreground font-[family-name:var(--font-heading)]">{t("galleryTitle")}</h2>
        </div>
        <a href="#" className="text-xs sm:text-sm text-primary hover:text-primary/80 font-medium transition-colors flex items-center gap-1 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary">
          {t("allPhotos")}
          <svg className="size-3.5 sm:size-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
      ...
      <div className="flex gap-1.5 sm:gap-2 mb-3 sm:mb-5 overflow-x-auto pb-1 scrollbar-hide" role="tablist" aria-label="Gallery categories">
        {categories.map((cat) => (
          <button
            ...
            className="px-3 sm:px-4 py-1.5 sm:py-2 rounded-full text-xs sm:text-sm font-medium whitespace-nowrap transition-all duration-200 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary touch-manipulation"
            style={{
              backgroundColor: activeCategory === cat.id ? "var(--primary)" : "var(--muted)",
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-1.5 sm:gap-3" role="tabpanel">
        {filteredImages.map((image) => (
          ...
          >
            <div className="absolute inset-0 bg-gradient-to-br from-primary/15 to-primary/5 group-hover:scale-105 transition-transform duration-500" />
            <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors duration-300 flex items-center justify-center">
              <div className="size-8 sm:size-10 rounded-full bg-white/90 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300">
